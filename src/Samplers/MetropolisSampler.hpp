...
class MetropolisSampler: public Sampler<F, coeffType> {
  public:
  ...
  MetropolisSampler(MetropolisSampler<F, coeffType> &&source):
    Sampler<F, coeffType>(static_cast<Sampler<F, coeffType> &>(source)){
    swap(*this,source);
    //setReference(getRandomDeterminant(*fullBasis));
    setReference(source.getDet());
  };
  ...
  friend void swap(MetropolisSampler<F, coeffType> &a, MetropolisSampler<F, coeffType> &b){
    swap(static_cast<Sampler<F, coeffType>&>(a),static_cast<Sampler<F, coeffType>&>(b));
  ...
  void setReference(detType const &a) {
    Sampler<F, coeffType>::setReference(a);
    lastCoeff = para->getCoeff(a);
  }
	void resetSpecs(){lastCoeff = para->getCoeff(Sampler<F, coeffType>::cDet);}
private:
  ...
  using Sampler<F, coeffType>::cDet;
};
